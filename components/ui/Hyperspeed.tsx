...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    // Use a temporary canvas to test WebGL compatibility before running the main App
    try {
      const tempCanvas = document.createElement('canvas');
      const gl = tempCanvas.getContext('webgl') || tempCanvas.getContext('experimental-webgl');
      if (!gl) {
        console.warn('WebGL is not supported in this browser/device. Hyperspeed animation is disabled.');
        return;
      }
    } catch (e) {
      console.warn('Failed to test WebGL for Hyperspeed:', e);
      return;
    }
    ...
    try {
      ...
      myApp.loadAssets().then(myApp.init).catch((err: any) => {
        console.warn('Failed to initialize Hyperspeed App:', err);
      });
    } catch (e) {
      console.warn('Failed to create Hyperspeed App:', e);
    }
